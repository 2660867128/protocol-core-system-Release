...
def setup_database():
    ...
    try:
        ...
        # 生成迁移文件（包含所有应用）
        apps = ["accounts", "connections", "wechat_login", "protocol_api",
                "read_check", "protocol_config", "updater", "auth_system", "auth_codes"]
        for app in apps:
            try:
...
def start_asgi_server():
    ...
    try:
        subprocess.run([sys.executable, "-m", "daphne", "-p",
                       str(port), "-b", host, "asgi:application"])
...
def start_django_server():
    ...
    if not check_port_available(host, port):
        print(f"⚠️  警告：端口 {port} 可能已被占用")
        print(f"💡 提示：如果上一个进程未正常退出，请等待几秒后重试")
    ...
    try:
        server_address = f"{host}:{port}"
        # 直接使用 subprocess.run 并实时输出
        process = subprocess.Popen(
            [sys.executable, "manage.py", "runserver", server_address],
        ...
        if return_code != 0:
            print(f"\n❌ 服务器启动失败，退出码: {return_code}")
            if return_code == 245:
                print("💡 提示：退出码 245 通常表示进程被信号终止")
                print("   可能原因：")
                print("   1. 端口被占用")
                print("   2. 权限不足")
                print("   3. 配置错误导致启动失败")
